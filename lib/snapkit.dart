...
class Snapkit {
  ...
  Future<void> share(
    ...
    if (mediaType == SnapchatMediaType.PHOTO) {
      ...
      ByteData byteData = File(image!).readAsBytesSync().buffer.asByteData();
      ByteBuffer buffer = byteData.buffer;

      File file = await new File('$path/image.jpg').writeAsBytes(
    ...
    if (mediaType == SnapchatMediaType.VIDEO) {
      ...
      String path = (await getApplicationDocumentsDirectory()).path;
      ByteData byteData =
          File(videoPath!).readAsBytesSync().buffer.asByteData();
